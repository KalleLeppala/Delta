...
dfo <- function(X, strict = FALSE) {
	...
	if (strict == FALSE) {
		L <- L + n(X, "ABABB") + n(X, "AAABB") + n(X, "BABAB")
		R <- R + n(X, "ABBAB") + n(X, "AABAB") + n(X, "BAABB")
	}
	value <- as.numeric((L - R)/(L + R))
	pvalue <- as.numeric(1 - pchisq(abs((L - R)**2/(L + R)), 1))
	if (is.numeric(pvalue) == TRUE && is.nan(pvalue) == FALSE) {
...
dil <- function(X, strict = FALSE) {
	...
	if (strict == FALSE) {
		L <- L + n(X, "BAABB") + n(X, "AAABB") + n(X, "ABBAB")
		R <- R + n(X, "BABAB") + n(X, "AABAB") + n(X, "ABABB")
	}
	value <- as.numeric((L - R)/(L + R))
	pvalue <- as.numeric(1 - pchisq(abs((L - R)**2/(L + R)), 1))
	if (is.numeric(pvalue) == TRUE && is.nan(pvalue) == FALSE) {
...
dfi <- function(X, strict = FALSE) {
	...
	if (strict == FALSE) {
		L <- L + n(X, "ABABB") + n(X, "ABAAB") + n(X, "BABAB")
		R <- R + n(X, "BAABB") + n(X, "BAAAB") + n(X, "ABBAB")
	}
	value <- as.numeric((L - R)/(L + R))
	pvalue <- as.numeric(1 - pchisq(abs((L - R)**2/(L + R)), 1))
	if (is.numeric(pvalue) == TRUE && is.nan(pvalue) == FALSE) {
...
dol <- function(X, strict = FALSE) {
	...
	if (strict == FALSE) {
		L <- L + n(X, "ABBAB") + n(X, "ABAAB") + n(X, "BAABB")
		R <- R + n(X, "BABAB") + n(X, "BAAAB") + n(X, "ABABB")
	}
	value <- as.numeric((L - R)/(L + R))
	pvalue <- as.numeric(1 - pchisq(abs((L - R)**2/(L + R)), 1))
	if (is.numeric(pvalue) == TRUE && is.nan(pvalue) == FALSE) {
...
These statistics are dominated by the singletons, and I don't think the predictions hold with specifically (malignously) designed graphs. Going to study what happens to `DFO()` and others with such a graph and a very big sample, later.

```{r singletons dominate DFOIL}
load("symmetric.RData")
print(tables[[1]])
```

The new categories don't all have the zero signal; in particular gene flow from the outgroup can be mixed up with an ancient gene flow event.

LOOKS LIKE "P34 -> P2" AND EVERYTHING INVOLVING "P1234" ARE ACTING STRANGE, IGNORE FOR NOW!

## An attempt to adapt DFOIL method to ancient samples

One of the issues we had with the new polar bear paper was the use of ancient sample despite the fact that DFOIL utilises private alleles. In chat proposed just dropping the private allele terms from the definition of the statistics, that is, using the functions `dfo()`, `dil()`, `dfi()`, `dol()` instead. Here I demonstrate that this will not work.

We could consider just using the new signs, but only if there's a reason they are stable within admixture categories, that they do not change if we make another admixture graph with different admixture proportions and branch lengths.
...
These are whatever, let's not care about them any further.
